...
def check_sources(problems: Problems, features: list[dict], quiet: bool) -> None:
    """Every feature's source is in exactly one place, and it says which.

    The distinction is who looks after the code, and this fork has three
    answers where the mod it was taken from had two:

      upstream/<Dir>   a submodule -- somebody else's mod, pinned to a
                       release, never edited here.  tools/sync.py moved the
                       pin and tools/build.py copied it into modules/.
      maintained/<Dir> source the tree owns, copied into modules/ the same
                       way.
      modules/<Dir>    the FORK's own answer: the source itself.

    The nightly channel is a fork, not a tracked build.  It carries neither
    sync.py nor build.py -- there is nothing here for them to pull from and
    nothing for them to assemble -- so `modules/` stopped being a build
    product and became the tree.  A feature with a directory there and
    nowhere else is that case and is fine; what is still an error is a
    feature in TWO of the three, which is ambiguous about which copy the game
    reads, and one in none of them, which is a feature with no code at all.

    tools/rebase.py in the channel's root is what a fork has instead of
    sync.py: it fetches two stable releases and offers the diff between them
    for merging into modules/, rather than overwriting a tree that has
    diverged on purpose.
    """
    tracked, owned, forked = 0, 0, 0
    ...
    for feature in sorted(features, key=lambda f: f["id"]):
        ...
        in_modules = (MODULES / directory).is_dir()
        ...
        elif not in_upstream and not in_maintained and not in_modules:
            problems.error(
                f"{label}: {directory!r} is under none of upstream/, "
                "maintained/ or modules/, so there is no source for it at "
                "all")
        elif in_maintained:
            owned += 1
            if not declared:
                problems.error(
                    ...
                    "does not say `maintained = true`; the README reads that "
                    "flag, not the directory")
        elif in_upstream:
            tracked += 1
            if declared:
                problems.error(
                    f"{label}: features.lua says `maintained = true` but the "
                    f"source is the submodule upstream/{directory}")
        else:
            forked += 1
            if declared:
                problems.error(
                    f"{label}: features.lua says `maintained = true` but "
                    f"there is no maintained/{directory}; in this fork the "
                    "source is modules/ and the flag says something that is "
                    "not true")
