...
archive, so this build carries the number and the caught marker carries the fix.

## [0.32.98] - 2026-09-07

Nothing in this mod changed. The channel ships one version across every
archive, so this build carries the number and the box carries the fix.
...
Nothing in this mod changed. The channel ships one version across every
archive, so this build carries the number and the POKéDEX carries the fix.
...
Nothing in this mod changed. The channel ships one version across every
archive, so this build carries the number and the two bundles carry the fixes.
...
- Version bump only; no Wild Green changes in 0.32.88.

## [0.32.87] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.87.

## [0.32.86] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.86.

## [0.32.85] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.85.

## [0.32.84] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.84.

## [0.32.83] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.83.

## [0.32.82] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.82.

## [0.32.81] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.81.

## [0.32.80] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.80.

## [0.32.79] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.79.

## [0.32.78] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.78.

## [0.32.77] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.77.

## [0.32.76] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.76.

## [0.32.75] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.75.

## [0.32.74] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.74.

## [0.32.73] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.73.

## [0.32.72] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.72.

## [0.32.71] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.71.

## [0.32.70] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.70.

## [0.32.69] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.69.

## [0.32.68] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.68.

## [0.32.67] - 2026-09-06

### Changed

- Version bump only; no Wild Green changes in 0.32.67.
...
- Version moved with the nightly channel; nothing else changed.
...
No change. Version-only, released alongside the UI and QOL bundles.
...
- **`PLAYER` takes effect where you are standing.** The row used to say "next
  launch" for the overworld walker, and a colour picker you have to reboot to
  see is a colour picker nobody turns twice.

  The reason it waited was real enough: the walker is a `sprites` record,
  records are folded into `Game.data` once at load, and `SpriteRenderer` copies
  the image out of the record when the `Player` is built. Neither had to stay
  settled. The recipe writes all nine suits at install — always, and it always
  has — so switching colour was never a question of generating anything, only
  of which path a record names. Turning the row repoints the folded record and
  rebuilds the renderers that had already copied out of it.

  `PORTRAIT SKIN` moves at the same moment and for the same reason. What still
  waits for a relaunch is boot data rather than art: the version ribbon's
  *artwork* and the name list. The ribbon's *colour* is live — that goes
  through the zone pass every frame.

  The `player.sprite` hook and the record sweep now install whatever the row
  says, `RED` included, because `RED` is a value the row can be moved *off* as
  well as onto and a hook that was never registered cannot be talked into
  existing later. Each declines on its own while the suit is `RED`, which is
  the same picture the old gate gave and one that can change its mind.

### Changed

- **The version ribbon follows `PLAYER`.** It used to stay green in every
  suit, on the grounds that `WILD GREEN VERSION` is the game's name and not
  the character's jacket. In front of a player who has just put the character
  in purple that reads as a setting that did not take. The words still say
  `GREEN`; only the ink moves.

  Each suit's band is two colours and they are not the suit's own three: the
  band is lettering on white, and at 8px a letter drawn in the outfit's own
  value washes out — which is what 1.0.0 shipped, and why green's pair was
  hand-sampled darker than green's outfit to begin with. So each pair is the
  outfit taken to a fixed *lightness*: the letter at 0.26 relative luminance
  and its one-pixel shadow at 0.45. Green's two are the hand-sampled pair the
  cartridge shell is named after and have not moved.

- **`MEWMON` takes the portrait ramp, not the overworld one.** That palette
  covers tile rows 10-17, and that band is not just the standing figure: the
  cycling mon, the POKE BALL beside him and the copyright line along the
  bottom are all inside it. Shade 2 is a light on every one of those and a
  face on none, so painting it the character's skin put a skin-coloured half
  on the ball and lettered `GAME FREAK`'s line in skin.

  The portrait ramp is already what the figure is baked to under `ADVANCED`,
  so the two modes now agree about him as well.

### Fixed

- **The title screen's two bands are coloured in every display mode.** They
  were registry overrides, and two display modes do not resolve named palettes
  through the registry at all: `PaletteFX.pal` short-circuits every name to
  the boot-ROM pair under `OG RED`, and reads `data/palettes_gbc` under
  `ADVANCED`.

  Under `ADVANCED` that pack's own `LOGO1` is white / `#f7f78c` / `#8cbd52` /
  `#ad0021`. The ribbon draws its letter in shade 2 and its shadow in shade 1,
  so `WILD GREEN VERSION` came out yellow-green on pale yellow — a yellow
  title on a green cart. Its `MEWMON` is white / `#ef9c6b` / `#7321a5` /
  black, and `#ef9c6b` is a skin tone: the ball's light half, and the
  highlight on the copyright line.

  Both bands are now recoloured in the frame's **zone list** as well, through
  `render.zones`, which is handed the finished list on the way to the blit
  after every name has been resolved. The bands are matched by rectangle
  rather than by palette identity — under `ADVANCED` the palette we would be
  looking for is the pack's and not ours — and by all three of the title's
  bands at once, which is a packet no other screen sends. The main menu's own
  box zone is not a band and keeps its black ink.

  The three deliberately monochrome modes (`OG`, `OG INV`, `CLASSIC`)
  substitute the whole screen downstream in `PaletteFX.effectiveColors` and
  are left to: a mono mode asked for one palette, not ours.

[stable]: https://github.com/wild1walker/Gen1MakeItGreen
