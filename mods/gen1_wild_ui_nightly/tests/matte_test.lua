...
-- ------- the title screen, on its own seam

io.write("the title's two rectangles are grounded from inside its draw\n")
do
  -- The two-pass wrapper cannot help here: TitleState:draw opens with a white
  -- fill of the WHOLE screen, so a matte painted between the passes is wiped
  -- by the second pass's own fill before any art lands on it.  The paint has
  -- to happen INSIDE the draw, and `currentSprite` is the one seam in there --
  -- called on the line before the mon is drawn.
  reset()
  local mon = { getDimensions = function() return 40, 48 end }
  local player = { getDimensions = function() return 40, 56 end }
  local base = function() return mon, true end
  local wrap = Matte.new(context).wrapTitleSprite(base)

  local image, trueColor = wrap({ player = player, monOffset = 0 })
  eq(image, mon, "the sprite is handed back untouched")
  eq(trueColor, true, "...and so is its true-colour answer")

  eq(#fills, 2, "two rectangles: the mon and the figure")
  -- the engine's own arithmetic: 40 + (56 - w) / 2 by 136 - h
  eq(fills[1].x, 48, "the mon, where TitleState draws it")
  eq(fills[1].y, 88, "...on the floor it stands on")
  eq(fills[1].w, 40, "at its own width")
  eq(fills[1].h, 48, "...and height")
  eq(fills[2].x, 82, "the figure, at the flat 82, 80 the engine uses")
  eq(fills[2].y, 80, "...both ways")
  eq(fills[1].colour[1], 0, "painted the colour that spot ends up")

  -- monOffset slides the mon during the cycle, and the rect goes with it
  reset()
  wrap({ player = player, monOffset = 24 })
  eq(fills[1].x, 72, "the mon's rect follows monOffset")
end

io.write("nothing is grounded where nothing is drawn\n")
do
  local player = { getDimensions = function() return 40, 56 end }
  local wrap = Matte.new(context).wrapTitleSprite(
    function() return { getDimensions = function() return 40, 48 end }, true end)

  -- MainMenu's ClearScreen has taken the art off the screen
  reset()
  wrap({ player = player, menuOpen = true })
  eq(#fills, 0, "the menu frame paints nothing")

  -- a sprite the engine will not mark is a sprite the shade pass colours,
  -- and a matte under it would be a hole
  reset()
  local plain = Matte.new(context).wrapTitleSprite(
    function() return { getDimensions = function() return 40, 48 end }, false end)
  plain({ player = player })
  eq(#fills, 1, "only the figure, which is marked either way")
  eq(fills[1].x, 82, "...at its own rect")

  -- and LIGHT pays nothing at all
  reset()
  themeValue = "light"
  wrap({ player = player })
  eq(#fills, 0, "LIGHT paints nothing")
  themeValue = "dark"
end
