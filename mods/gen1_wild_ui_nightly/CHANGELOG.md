...
- **`tools/make_gen2_towns.py`: the town colours, generated.**
  ...
  uses. The input is `data/generated/palettes.lua` from the player's own
  cartridge import, so it is not in this repository and `og/gen2/` is not
  committed; until it is run, a town takes the plain scene. Kanto is
  regenerated rather than shared with Red's eleven folders, because Gold
  repaints Kanto.
